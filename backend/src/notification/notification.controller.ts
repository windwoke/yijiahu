...
export class NotificationController {
  ...
  async list(@Req() req: any, @Query() q: NotificationQueryDto) {
    ...
    return this.svc.findByUser(req.user.id, q.page, q.pageSize, q.type, isRead, q.familyId);
  }
  ...
  /** 标记单条已读 */
  @Put(':id/read')
  async markRead(@Req() req: any, @Param('id') id: string, @Query('familyId') familyId?: string) {
  ...
  /** 删除通知 */
  @Delete(':id')
  async delete(@Req() req: any, @Param('id') id: string, @Query('familyId') familyId?: string) {
