...
export default function DailyCarePage() {
  ...
  const [medCompleted, setMedCompleted] = useState<number | null>(null);
  const [medTotal, setMedTotal] = useState<number | null>(null);
  ...
  const loadData = async () => {
    ...
    try {
      // 并行请求：今日用药汇总（主要来源）+ 今日打卡数据（状态和备注）
      const [medSummary, checkinsData] = await Promise.all([
      ...
      // 用药进度始终以 medication summary 为准（和首页一致）
      if (medSummary) {
  ...
  // 用药进度
  const progress = medTotal != null && medTotal > 0 && medCompleted != null
    ? Math.min(100, Math.round((medCompleted / medTotal) * 100))
    : 0;
          ...
          <View className="med-card">
            <View className="med-header">
              ...
              <Text className="med-count">
                {medTotal === null ? '加载中...' : medTotal > 0 ? `${medCompleted} / ${medTotal} 项` : '暂无药品'}
              </Text>
            </View>
            {medTotal !== null && medTotal > 0 && (
